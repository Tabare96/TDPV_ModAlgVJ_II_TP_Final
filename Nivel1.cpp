...
Nivel1::Nivel1(Vector2i resolucion, const std::string& titulo) : NivelBase()
{
	fps = 120;
	tiempoFrame = 1 / 120.f;
	tiempo2 = 0;
	...
	this->ragdollsRestantes = limiteRagdolls; // Inicializa en 4
	...
	mundo1->SetContactListener(this); // Asigna el listener de colisiones
...
void Nivel1::IniciarFisica()
{
	...
	//Plataformas 	   				             posx, posY, ancho, alto
...
void Nivel1::BeginContact(b2Contact* contact)
{
	...
	for (Caja* caja : cajas)
	{
		...
		{
			// Verifica si alguna parte del ragdoll colisionó
			...
			for (Ragdoll* ragdoll : ragdolls)
			{
				...
				if (colisionConRagdoll) break; // Rompe el bucle si ya hubo una colisión con alguna parte
			}

			// Solo incrementa si la colisión fue con un ragdoll
			if (colisionConRagdoll)
			{
				...
				break; // Terminar para evitar contar la misma caja más de una vez
			}
		}
	}
}
